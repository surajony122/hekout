  ...
  window.CheckoutFlow = {
    ...
    open: async function(options) {
      ...
      let widgetConfig = { 
        ...
        primaryColor: '#111827',
        preLoginBannerText: '≡ƒÄë FREE SHIPPING ON ALL ORDERS TODAY!', preLoginBannerBg: '#000000', preLoginBannerColor: '#ffffff',
        postLoginBannerText: 'ΓÜí EXTRA 2% OFF ON UPI/CARDS', postLoginBannerBg: '#ecfdf5', postLoginBannerColor: '#059669'
      };
        ...
        <div id="cf-sheet" style="display:flex; flex-direction:column; height:100%;">
          ...
          <div style="background:#fff; padding:16px 20px; display:flex; justify-content:space-between; align-items:center; border-top-left-radius:24px; border-top-right-radius:24px; border-bottom:1px solid #f3f4f6;">
            ...
            <div style="text-align:right;">
              <div style="font-weight:700; font-size:1.1rem; color:#111;" id="cf-header-total">Γé╣${total}</div>
            </div>
          </div>
          ...
          <div style="flex:1; overflow-y:auto; padding:20px; background:#f9fafb;">
              ...
              <div id="cf-accordion-body" style="padding:0 20px 20px 20px; display:none; border-top:1px solid #f3f4f6;">
                <div style="display:flex; gap:16px; align-items:flex-start; margin-top:20px;">
                  ...
                  <div style="flex:1;">
                    <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                      ...
                      <div style="text-align:right;">
                        <div style="font-weight:700; font-size:1rem; color:#111827;" id="cf-item-total">Γé╣${total}</div>
                      </div>
                    </div>
                ...
                <div style="margin-top:20px; font-size:0.95rem; color:#4b5563;">
                  <div style="display:flex; justify-content:space-between; margin-bottom:10px;">
                    <span>Subtotal</span>
                    <span id="cf-summary-subtotal" style="font-weight:500;">Γé╣${total}</span>
                  </div>
                  <div id="cf-summary-discount-row" style="display:none; justify-content:space-between; margin-bottom:10px; color:#059669; font-weight:600;">
                    <span>Discount</span>
                    <span id="cf-summary-discount-value">-Γé╣0</span>
                  </div>
                  ...
                  <div style="display:flex; justify-content:space-between; font-weight:700; font-size:1.1rem; color:#111827;">
                    <span>Total</span>
                    <span id="cf-summary-total">Γé╣${total}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div id="cf-offers-section">
              ...
              <div id="cf-discount-msg" style="color:#059669; font-size:0.85rem; font-weight:500; margin-top:-10px; margin-bottom:16px; padding-left:8px; display:${appliedDiscount ? 'block' : 'none'};">≡ƒÄë Auto Discount Applied!</div>
            </div>
            ...
            <div id="cf-phone-step">
              <div class="cf-card" style="display:flex; align-items:center; padding:0 16px; height:56px; margin-bottom:16px;">
                <div style="display:flex; align-items:center; gap:8px; border-right:1px solid #e5e7eb; padding-right:16px; height:100%;">
                  <span style="font-size:1.2rem;">≡ƒç«≡ƒç│</span>
      ...
      const goToCheckoutScreen = async () => {
        ...
        promoBanner.innerText = widgetConfig.postLoginBannerText || 'ΓÜí EXTRA 2% OFF ON UPI/CARDS';
      ...
      const renderPaymentOptions = () => {
        ...
        const getDiscountTag = () => {
          ...
          const txt = widgetConfig.prepaidDiscountType === 'percentage' ? `${widgetConfig.prepaidDiscountValue}%` : `Γé╣${widgetConfig.prepaidDiscountValue}`;
        ...
        if (widgetConfig.isPartialCodEnabled) {
          html += renderOption('PartialCOD', 'Partial COD', `Pay Γé╣${widgetConfig.partialCodAmount} now`, icons.cod, false, false);
        }
      ...
      window.internalUpdatePricing = () => {
        ...
        priceEls.forEach(el => {
          ...
          if (['UPI', 'Card', 'Wallet', 'Netbanking'].includes(val) && widgetConfig.isPrepaidDiscountEnabled) {
            ...
            el.innerText = `Γé╣${Math.max(0, subtotal - discountAmount - pd).toLocaleString('en-IN')}`;
          } else if (val === 'PartialCOD') {
            el.innerText = `Γé╣${widgetConfig.partialCodAmount}`;
          } else {
            el.innerText = `Γé╣${Math.max(0, subtotal - discountAmount).toLocaleString('en-IN')}`;
          }
        });

        const finalTotal = Math.max(0, subtotal - discountAmount);
        document.getElementById('cf-item-total').innerText = `Γé╣${subtotal.toLocaleString('en-IN')}`;
        ...
        document.getElementById('cf-summary-subtotal').innerText = `Γé╣${subtotal.toLocaleString('en-IN')}`;
        
        if (discountAmount > 0) {
          document.getElementById('cf-summary-discount-row').style.display = 'flex';
          document.getElementById('cf-summary-discount-value').innerText = `-Γé╣${discountAmount.toLocaleString('en-IN')}`;
        } else {
          document.getElementById('cf-summary-discount-row').style.display = 'none';
        }
        
        document.getElementById('cf-summary-total').innerText = `Γé╣${finalTotal.toLocaleString('en-IN')}`;
        document.getElementById('cf-header-total').innerText = `Γé╣${finalTotal.toLocaleString('en-IN')}`;
      };
      ...
      document.getElementById('cf-apply-discount').onclick = async () => {
        ...
        try {
          ...
          if (data.success && data.valid) {
            ...
            msgEl.innerText = "≡ƒÄë Coupon Applied!";
              ...
              if (data.orderStatusUrl) {
                ...
                sheet.innerHTML = `
                  <div style="text-align:center; padding: 60px 20px;">
                    <div style="font-size:4rem; color:#10b981; margin-bottom:20px;">Γ£ô</div>
