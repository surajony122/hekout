...
export default function CustomersClient({ customers }: { customers: any[] }) {
  ...
  const handleExport = () => {
    ...
    const headers = 'Customer Name,Phone,Email,City,State,Status,Last Order Date\n';
    const csv = filteredCustomers.map(c => {
      const lastOrder = c.lastOrderAt ? new Date(c.lastOrderAt).toLocaleDateString('en-GB') : '';
      return `"${c.name || ''}","${c.phone || ''}","${c.email || ''}","${c.city || ''}","${c.state || ''}","${c.verificationStatus || ''}","${lastOrder}"`;
    }).join('\n');
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">{filteredCustomers.length} total customers</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" size="sm" className="bg-white shadow-sm" onClick={handleExport}>
